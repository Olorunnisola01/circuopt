...
\begin{abstract}
\noindent
Circular ecodesign is often constrained by limited quantitative evidence that
circular design strategies can deliver both environmental and economic
benefits. This study develops and applies a three-objective optimisation
framework based entirely on publicly available data, using the GARO LS4
ground-mounted AC electric-vehicle charging station as a case study. Product
mass, bill of materials, charging throughput, and service-life data were
obtained from a published manufacturer life-cycle assessment. Fourteen design
variables were optimised across three objectives: cradle-to-grave carbon
emissions per service-year (\emph{Planet}), annualised life-cycle cost
(\emph{Prosperity}), and a composite index incorporating repairability,
modularity, circular employment, and supply risk (\emph{People}).

The framework integrates process-level environmental data from Idemat 2026
and commodity prices from the USGS Mineral Commodity Summaries 2025, while
explicitly identifying proxies and assumptions where publicly available
empirical data were unavailable. NSGA-III was used to generate a Pareto
archive of 359 non-dominated designs. All 359 designs outperformed a linear
take-make-waste baseline in both carbon emissions and cost. The
best-performing solutions reduced carbon emissions per service-year by up to
62.6\,\% and costs by up to 22.7\,\%.

Robustness was evaluated using 500 Monte Carlo scenarios representing
uncertainty in grid decarbonisation, carbon pricing, commodity prices,
remanufacturing performance, and prospective recycled-content requirements.
Of the 359 Pareto-optimal designs, 55 remained robust across the simulated
futures. Sensitivity analyses examined the effects of impact category,
end-of-life allocation, grid geography, social weighting, price source,
electronics modelling, recyclate assumptions, and uncertainty-distribution
widths. The results showed that methodological choices, particularly grid
geography and inventory selection, could influence the Planet objective more
strongly than variation among the Pareto-optimal designs themselves.

Finally, applying four stakeholder preference profiles to the same Pareto
front produced three distinct design recommendations, demonstrating that
optimisation identifies feasible trade-offs rather than a universally optimal
solution. The framework provides a transparent and reproducible approach for
distinguishing circular designs that perform well under current assumptions
from those that remain effective under uncertainty.
\end{abstract}

\section{Why This Problem Matters}

The circular economy remains a contested concept. A review of 114 published
definitions found that most focus primarily on reducing, reusing, and
recycling materials, while giving limited consideration to social equity
[24]. To address this limitation, the present framework treats the People
dimension as an independent optimisation objective rather than as a
constraint appended to an environmental model. This approach is consistent
with Life Cycle Sustainability Assessment, which integrates environmental
life-cycle assessment (LCA), life-cycle costing (LCC), and social
life-cycle assessment (S-LCA) as three complementary and equally important
dimensions [22].

The European Union's Ecodesign for Sustainable Products Regulation shifts
circular product design from a largely voluntary practice towards a
regulatory requirement [11]. Its introduction of the Digital Product
Passport is also expected to improve the availability and traceability of
product information throughout the life cycle, including at end of life,
beyond the capabilities for which existing Waste Electrical and Electronic
Equipment take-back schemes were originally designed [27].

The principal barrier to circular design is therefore no longer a lack of
intent. More comprehensive circular strategies, including design for
disassembly, remanufacturing, and high-purity recycling, often require
additional initial investment. Their benefits may not be realised for many
years, may accrue to different actors across the value chain, and may
depend on uncertain commodity prices, technological developments, and
regulatory conditions. Consequently, a product manager deciding whether to
approve an immediate tooling premium for a future material-recovery benefit
lacks a sufficiently rigorous basis for evaluating the trade-off.

Existing eco-efficiency tools often address a narrower problem. They may
optimise a single variable while holding other design choices constant,
evaluate performance using present-day conditions, and give limited
consideration to social outcomes. Although such analyses can produce
locally optimal recommendations, they may provide insufficient support for
long-term strategic decisions.

This study adopts a different approach based on three principles.

\begin{enumerate}
\item \textbf{Three objectives rather than one.} Planet, Prosperity, and
People are retained as separate objectives throughout the optimisation.
Premature aggregation could obscure the trade-offs that decision-makers
need to evaluate explicitly.
\item \textbf{Future scenarios rather than a single snapshot.} Designs are
evaluated according to how consistently they perform across an ensemble of
plausible futures, rather than solely by their ranking under present-day
conditions.
\item \textbf{Transparent provenance rather than false precision.} Each
model input is classified as empirical data, a proxy, or an assumption.
Proxies and assumptions are subjected to greater variation in the scenario
analysis, allowing readers to distinguish conclusions supported by
measurement from those that remain dependent on modelling judgement.
\end{enumerate}
...
which matters for two reasons beyond the general benefit of a more detailed
inventory.
...
location, which is the one geographic assumption grounded in direct evidence.
...
($\sigma=0.35$ lognormal) in the ensemble. Omitting this classification would
present these values with an unwarranted appearance of certainty.
...
The product investigated in this study is the GARO LS4, a ground-mounted AC
electric-vehicle charging station with a total mass of \SI{24.50}{\kilo\gram}.
...
\begin{figure}[h]
\centering
\includegraphics[height=6.5cm]{figures/fig_garo_ls4.jpg}
\caption{The GARO LS4 ground-mounted AC electric-vehicle charging station,
the case product analysed throughout this report. Image: GARO AB,
\emph{LS4 Public Charging}, \url{https://garocharging.com/ls4-public-charging/}
(accessed 2026), reproduced for identification of the product under study.}
\label{fig:garo_ls4}
\end{figure}
...
Two aspects of the mass allocation are this project's own assumptions rather
than part of the cited source. Packaging (corrugated cardboard,
...
$\eta$ the conversion loss. Dividing by $S$ captures the central mechanism of
the model: it allows a longer-lived, remanufacturable design to amortise a
heavier embodied burden, and it is why lifetime and recycled content compete
...
\paragraph{People.} The People objective addresses dimensions that resist
direct quantification and is therefore treated as a composite of four
normalised terms rather than a single invented metric:
\begin{equation}
...
\paragraph{Recycled-content cost penalty.} An early version of this
model drove recycled content to its upper bound in every non-dominated design.
This behaviour was not a genuine finding but a modelling artefact: the DEFRA closed-loop factor and the
...
often a design remains non-dominated inside the archive is largely
uninformative here, because the archive is \emph{already} a mutually non-dominated set:
...
Indicator of 0.00, correctly reflecting a design that is neither made of nor
destined for recovered material.
...
Conversion efficiency, not standby draw, is therefore the dominant contributor
to use-phase carbon, a pattern the optimiser identifies without being
explicitly directed toward it.

\FloatBarrier
\subsection{Comparison with the Published Life-Cycle Assessment}
...
comparable to
the sampling error of the estimator itself, indicating a stable front rather
than an artefact of stochastic sampling. Equal-budget random search reaches
93.2\% of the same
hypervolume. That gap quantifies the algorithm's specific contribution,
...
This result addresses the common concern that circular design strategies are
necessarily cost-prohibitive: the additional capital cost of circularity in
this product is recovered through amortisation over an extended service life.
A longer, remanufacturable life spreads the embodied burden over more service-years, and
...
the best design. \textbf{The selection of the best-performing design was more
sensitive to the allocation convention than to variation within the design
space.} Any result in this report that concerns end-of-life route
...
rank correlation across the archive stays above $+0.99$ everywhere. The
absolute magnitude of the Planet objective is therefore highly sensitive to
the assumed grid, whereas the \emph{ranking} of
...
design: the designs identified here that reduce both carbon emissions and
cost simultaneously are real and reproducible, but it is a reason to report the modelling choices with the same weight as the
design recommendation, which is what Sections~7 and~9 of this report do rather
than treating them as footnotes to it.

\FloatBarrier
\section{NSGA-III versus NSGA-II and Random Search}
...
\FloatBarrier
\section{Decomposition of the Validation Gap by Material Family}
...
This choice also results in closer agreement with the published study's own
...
The central finding (that designs reducing both carbon and cost simultaneously
exist and can be found systematically) is a statement about the
\emph{structure} of the
...
\item Product photograph, Figure~\ref{fig:garo_ls4}: GARO AB, \emph{LS4 Public
Charging}, \url{https://garocharging.com/ls4-public-charging/}, accessed 2026.
Reproduced for identification of the product under study; copyright remains
with GARO AB.
\end{itemize}
